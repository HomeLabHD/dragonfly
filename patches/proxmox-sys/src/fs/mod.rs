...
/// Information about a mounted file system from statfs syscall
pub struct FileSystemInformation {
    ...
    pub free_inodes: u64,
    /// the type of the filesystem (Linux-only, 0 on macOS)
    pub fs_type: i64,
    /// the filesystem id
    pub fs_id: libc::fsid_t,
}

/// Get file system information from path (Linux version using statfs64)
#[cfg(target_os = "linux")]
...
/// Get file system information from path (macOS version using statfs)
#[cfg(not(target_os = "linux"))]
pub fn fs_info<P: ?Sized + nix::NixPath>(path: &P) -> nix::Result<FileSystemInformation> {
    let mut stat: libc::statfs = unsafe { std::mem::zeroed() };

    let res = path.with_nix_path(|cstr| unsafe { libc::statfs(cstr.as_ptr(), &mut stat) })?;
    nix::errno::Errno::result(res)?;
    ...
    Ok(FileSystemInformation {
        ...
        total_inodes: stat.f_files,
        free_inodes: stat.f_ffree,
        fs_type: 0, // macOS doesn't have f_type in statfs
        fs_id: stat.f_fsid,
    })
}
